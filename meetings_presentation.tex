...
\tableofcontents % Throughout your presentation, if you choose to use \section{} and \section{} commands, these will automatically be printed on this slide as an overview of your presentation
\end{frame}
...
\section{Adapted PINNs for Crystal Growth}
\section{PINNs for Cryogenic Systems}
\section{Dyad Agent Framework}
...
%=====================================================
\subsection{XPINN: Domain Decomposition}

\begin{frame}{Motivation for XPINN}
\begin{itemize}
\item Global PINNs struggle with large domains
\item Boundary layers are spatially localized
\end{itemize}
\end{frame}

\begin{frame}{XPINN Concept}
\begin{itemize}
\item Decompose domain into subdomains
\item One neural network per subdomain
\item Interface conditions enforce consistency
\end{itemize}
\end{frame}

\begin{frame}{XPINN Domain Decomposition}
\placeholder{Melt Domain Split into Multiple Subdomains}
\end{frame}

\begin{frame}{Interface Loss Terms}
\[
\mathcal{L}_{\text{int}}
= \|u^{(1)} - u^{(2)}\|^2
+ \|\nabla u^{(1)} - \nabla u^{(2)}\|^2
...
\begin{frame}{Why XPINN Improves Stability}
\begin{itemize}
\item Localizes stiffness
\item Reduces spectral bias
\item Mimics FEM element locality
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Improved PINNs handle thermal--fluid coupling
\item SI embedding improves spatial accuracy
\item XPINNs improve stability at high Ra
\item Results comparable to COMSOL
\end{itemize}
\end{frame}
